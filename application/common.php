...
/**
 *  品牌页缩略图 给于标签调用 拿出商品表的 thumb 原始图来裁切出来的
 * @param type $goods_id  商品id
 * @param type $width     生成缩略图的宽度
 * @param type $height    生成缩略图的高度
 */
function brand_thum_images($goods_id,$width,$height){
    if(empty($goods_id))
        return '';
    //判断缩略图是否存在
    $path = "public/thumb/brand/" . date('Y-m-d',time()) . "/$goods_id/";
    $goods_thumb_name ="brand_thumb_{$goods_id}_{$width}_{$height}";
    ...
    $original_img = M('Brand')->where("id", $goods_id)->getField('logo');
