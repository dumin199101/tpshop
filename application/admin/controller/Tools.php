    ...
    public function import($time = 0, $part = null, $start = null){
        ...
        if(is_numeric($time) && is_null($part) && is_null($start)){ //初始化
            ...
            if(count($list) === $last[0]){
                ...
                return json(array('data'=>array('part' => 1, 'start' => 0), 'info'=>'初始化完成！', 'status'=>1, 'url'=>''));
            } else {
                return json(array('info'=>'备份文件可能已经损坏，请检查！', 'status'=>0, 'url'=>''));
            }
            ...
            if(false === $start){
                return json(array('info'=>'还原数据出错！', 'status'=>0, 'url'=>''));
            } elseif(0 === $start) { //下一卷
                if(isset($list[++$part])){
                ...
                if($start[1]){
                    $rate = floor(100 * ($start[0] / $start[1]));
                    return json(array('data'=>array('part' => $part, 'start' => $start[0]), 'info'=>"正在还原...#{$part} ({$rate}%)", 'status'=>1, 'url'=>''));
                } else {
                    return json(array('data'=>array('part' => $part, 'start' => $start[0]),'gz'=>1, 'info'=>"正在还原...#{$part}", 'status'=>1, 'url'=>''));
                }
            }
        } else {
            return json(array('info'=>'参数错误！', 'status'=>0, 'url'=>''));
